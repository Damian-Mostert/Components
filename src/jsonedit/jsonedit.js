            ...
            if (resolve.checked) {
                ...
                setData(Function(`
                    arguments[0]${built}.splice(${index + 1},0,arguments[1]);
                     return arguments[0];
                `)(Remake(Global), resolve.checked), "insert", Key, index, rules);
            }
        })
    };
...
function ObjectElement({ z, variant, button, Permisions, textNotEditable, Global, setData, Data, Level, Id, Key }) {
    ...
    if (Config?.Component) {
        let NewData = {};
        for (let key in Data) if (key != "__Hide") NewData[key] = Data[key];
        switch (Config.Component) {
            case "Accordion":
                return <Accordion variant={button} tabs={Object.keys(NewData).map((keyName) => {
                    let TypeNameIn;
                    for (let t of types)
                        if (t[keyName]) TypeNameIn = t[keyName];
                    if (keyName == "__Hide" || keyName == "__KeyClassName") return <span key={Key + "-:::-" + keyName} />

                    return Build({ TypeName: TypeNameIn, variant, button, Permisions, textNotEditable: textNotEditable || Config ? !Config?.edit : false, z, Global, setData, Data: Data[keyName], Level: Level + 1, Index: keyName, Id: Id + "-" + keyName, Key: Key + "-:::-" + keyName })

                })} indexes={Object.keys(NewData)} tab={Object.keys(NewData)[0]} />
            case "Nav":
                return <Nav variant={button} tabs={Object.keys(NewData).map((keyName) => {
                    let TypeNameIn;
                    for (let t of types)
                        if (t[keyName]) TypeNameIn = t[keyName];
                    if (keyName == "__Hide" || keyName == "__KeyClassName") return <span key={Key + "-:::-" + keyName} />
...
export default function JsonEdit({ DataIn, variant, buttons, devSites = [], onSave = () => { }, Permissions, setWarnOnExit = () => { } }) {
                ...
                case "review-dev":
                    return <>
                        <div className={"editer-" + variant}>
                            <Nav indexes={devSites.map((item) => (item.title))} tab={devSites?.[0]?.title} tabs={devSites.map((item) => {
                                return <iframe className="w-full h-full" src={item.src} />
                            })} variant={buttons} />
                        </div>
                        <div className="w-full flex min-h-[50px] justify-start p-4">
                            <Button label="back" variant={buttons} onClick={() => setTab("normal")} />
                        </div>
                    </>
                case "json":
                    return <>
                ...
                case "normal":
                    return <>
                        <div className="w-full flex h-full overflow-auto">
                            ...
                            {showHistory && <Resizable defaultSize={{ width: 400 }} className={`flex h-full editer-${variant}-history !overflow-hidden flex flex-col max-h-full`}>
                        ...
                        <div className="w-full flex justify-end p-4">
                            ...
                            <Button label="view dev" variant={buttons} onClick={() => setTab("review-dev")} />
                        </div>
                    </>
            }
        })()}
